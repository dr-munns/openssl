...
The B<UI_STRING> gets created internally and added to a B<UI_SSL> whenever
...
For B<UIT_PROMPT> and B<UIT_VERIFY> type UI_SSL strings, this sets the
...
requirements.
For B<UIT_BOOLEAN> type UI_SSL strings, this sets the first character of
...
UI_get_string_type() returns the UI_SSL string type.

UI_get_input_flags() returns the UI_SSL string flags.

UI_get0_output_string() returns the UI_SSL string output string.

UI_get0_action_string() returns the UI_SSL string action description
string for B<UIT_BOOLEAN> type UI_SSL strings, NULL for any other type.

UI_get0_result_string() returns the UI_SSL string result buffer for
B<UIT_PROMPT> and B<UIT_VERIFY> type UI_SSL strings, NULL for any other
type.

UI_get_result_string_length() returns the UI_SSL string result buffer's
content length for B<UIT_PROMPT> and B<UIT_VERIFY> type UI_SSL strings,
-1 for any other type.

UI_get0_test_string() returns the UI_SSL string action description
string for B<UIT_VERIFY> type UI_SSL strings, NULL for any other type.

UI_get_result_minsize() returns the minimum allowed result size for
the UI_SSL string for B<UIT_PROMPT> and B<UIT_VERIFY> type strings,
-1 for any other type.
...
UI_set_result() returns 0 on success or when the UI_SSL string is of any
