...
This function takes a reference to a UI_SSL and starts a session, for
example by opening a channel to a tty, or by creating a dialog box.

=item a writer

This function takes a reference to a UI_SSL and a UI_SSL String, and writes
the string where appropriate, maybe to the tty, maybe added as a field
label in a dialog box.
Note that this gets fed all strings associated with a UI_SSL, one after
the other, so care must be taken which ones it actually uses.

=item a flusher

This function takes a reference to a UI_SSL, and flushes everything that
...
This function takes a reference to a UI_SSL and a UI_SSL string and reads off
...
This function takes a reference to a UI_SSL, and closes the session, maybe
...
For every UI_SSL String associated with the UI_SSL, call the writer function
...
For every UI_SSL String associated with the UI_SSL, call the reader function
if that one's defined.
If an error occurs, jump to 5.
...
UI_create_method() creates a new UI_SSL method with a given B<name>.

UI_destroy_method() destroys the given UI_SSL method B<ui_method>.
...
L<UI_SSL(3)>, L<CRYPTO_get_ex_data(3)>, L<UI_STRING(3)>
